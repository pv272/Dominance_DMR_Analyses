...
#I need the weight for wehen I will need to bind to the closest weight 
Weight<-con %>% 
...
SubCall_All<-FocalCall_Tidy %>% 
  ...
  ungroup() %>% 
  arrange(ObsDate) %>% #arrange the observation in ascending order
  mutate(InteractionRef = 1:n()) %>%  #unique idendity for each interaction and that reflects the chronological sequence
  group_by(Colony) %>% 
  ...
  mutate(InterractionType=ifelse(SexLoser =="F" & SexWinner=="F","Female",
     ifelse (SexWinner=="M" & SexLoser =="M","Male", "Mixed"))) #characterize the type of interaction 
str(SubCall_All)
  names(SubCall_All)
  View(SubCall_All)

#I need to add how many times each pair has intercated with each other 
#I need to know how many times each animals has interacted 
  #mutate(WinnerInteraction_Count = map_dbl(Winner,~sum(.x)) need check the map shit 
  

write.csv(SubCall_All,"SubCall.csv",row.names = FALSE)#that changes the format of some variable 
...
WeightList<-Weight %>% 
  select(AnimalID,WeightDate,Weight)

#get the winner weight 
WinnerList<-SubCall %>% 
  select(ObsDate,Winner) %>% 
  distinct(Winner,ObsDate)

WinnerWeight<- inner_join(WinnerList,WeightList, by=c("Winner" = "AnimalID")) %>%
mutate(DayDiff = abs(round(difftime(WeightDate,ObsDate,units="days")))) %>% 
group_by(Winner,ObsDate) %>% 
filter(DayDiff == min(DayDiff)) %>%
ungroup() %>% 
group_by(Winner,ObsDate,DayDiff) %>% 
...
#get the loser weight

LoserList <- SubCall %>% 
  select(ObsDate,Loser) %>% 
  distinct(Loser,ObsDate)
