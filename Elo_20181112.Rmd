...
Eloscore could also be computed using passes interactions yet the dataset has to be prepared. 
...
After Elo has been calculated, one has to organize dataframe in anticipation of statistical analyses and the Elo score that we may want to retain. One should:
1) add the ratio of interaction per individuals: for each interaction the ratio is of 1:current group size. One can sum the ratio of each interaction to calculate the cumulative ration of interaction/individual. Sanchez 2016 shows that a ratio of 10 is good for steep hierarchy
2) Add the number of interactions for each given pairs (colin has to provide code since he did that already)
3) Add the number of interactions each individual was involved in 


#statistical inferences on explanatory variables of Elo score
